...
export const cooldownsByUserAndProvider = new Map<string, ProviderCooldown>();

export function getCooldownKey(userId: string, provider: string): string {
  return `${userId}:${provider}`;
}

export function getCooldown(userId: string, provider: string): ProviderCooldown | null {
  const key = getCooldownKey(userId, provider);
  const cd = cooldownsByUserAndProvider.get(key);
  if (!cd) return null;
  if (cd.cooldownUntil.getTime() <= Date.now()) {
    cooldownsByUserAndProvider.delete(key);
    return null;
  }
  return cd;
}
...
export function setCooldown(
  ...
  const key = getCooldownKey(userId, provider);
  cooldownsByUserAndProvider.set(key, {
    cooldownUntil: new Date(Date.now() + durationMs),
    reason,
  });
}

export function clearCooldown(userId: string, provider?: string): void {
  if (provider) {
    cooldownsByUserAndProvider.delete(getCooldownKey(userId, provider));
  } else {
    for (const key of cooldownsByUserAndProvider.keys()) {
...
export function shouldFailover(error: any): boolean {
...
export async function getOrUpdateApiStatus(userId: string): Promise<ApiStatusResponse> {
  ...
  if (primaryCooldown) {
    const primaryCdSec = Math.max(
      0,
      Math.ceil((primaryCooldown.cooldownUntil.getTime() - now.getTime()) / 1000)
    );
    ...
    primaryStatus: {
      ...
      cooldownRemainingSeconds: getCooldown(userId, creds.primaryProvider)
        ? Math.max(
            0,
            Math.ceil(
              (getCooldown(userId, creds.primaryProvider)!.cooldownUntil.getTime() - now.getTime()) /
                1000
            )
          )
        : undefined,
...
export async function fetchFromAlphaVantage(symbol: string, apiKey: string): Promise<TickerData> {
  ...
  try {
    ...
    if (!quote || Object.keys(quote).length === 0) {
      const errorMsg =
        data['Note'] || data['Information'] || data['Error Message'] || 'Invalid response from Alpha Vantage';
      ...
      if (
        ...
        throw new Error(
          'Alpha Vantage rate limit reached: Our standard API call frequency is 25 requests per day. Please wait a moment before trying again.'
        );
      }
      if (
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        throw new Error(
          'Polygon.io rate limit reached: Maximum number of requests allowed per minute (5/min). Please try again in a few moments.'
        );
      }
...
export async function fetchWithFailover(
  ...
  if (primaryCooldown) {
    console.warn(
      `[PriceFeedService] Primary provider (${primaryProvider}) in cooldown until ${primaryCooldown.cooldownUntil.toISOString()}.`
    );
    ...
    if (autoSwitchOnRateLimit && backupProvider && backupKey) {
      ...
      if (backupCooldown) {
        throw new Error(
          `Both ${primaryProvider} and ${backupProvider} are currently rate limited. ` +
      ...
      console.log(
        `[PriceFeedService] Cooldown active for ${primaryProvider}. Routing request for ${symbol} directly to backup (${backupProvider}).`
      );
      try {
      ...
      const isNetwork = primaryCooldown.reason.toLowerCase().includes('network');
      throw new Error(
        `Primary provider (${primaryProvider}) is ${isNetwork ? 'experiencing connection errors' : 'rate limited'}. Failover unavailable (${
          !autoSwitchOnRateLimit ? 'auto-switch disabled' : 'no backup key configured'
        }).`
      );
    }
  }
  ...
  if (!primaryKey) {
    if (autoSwitchOnRateLimit && backupProvider && backupKey) {
      ...
      if (!backupCooldown) {
        console.log(
          `[PriceFeedService] No primary key for ${primaryProvider}. Routing to backup ${backupProvider}.`
        );
        const tickerData = await callProvider(backupProvider, backupKey);
        return {
          tickerData,
  ...
  try {
    ...
    if (shouldFailover(primaryErr)) {
      ...
      console.warn(
        `[PriceFeedService] ${isRate ? 'Rate limit' : 'Network error'} hit on ${primaryProvider}: ${primaryErr.message}. Activating 60s cooldown.`
      );
      ...
      if (autoSwitchOnRateLimit && backupProvider && backupKey) {
        ...
        if (!backupCooldown) {
          console.log(
            `[PriceFeedService] Seamlessly failing over to backup provider (${backupProvider}) for ${symbol}...`
          );
          try {
            const tickerData = await callProvider(backupProvider, backupKey);
            return {
            ...
            throw new Error(
              `Failover failed: Primary ${isRate ? 'rate limited' : 'network error'} (${primaryErr.message}), and backup also failed (${backupErr.message}).`
            );
          }
        } else {
          throw new Error(
            `Primary provider ${isRate ? 'rate limited' : 'encountered network error'} (${primaryErr.message}), and backup is already in cooldown.`
          );
        }
      }
    }
...
export async function getLivePriceForStock(
  ...
  try {
    ...
    console.log(
      `[PriceFeedService] Live price cached for ${stock.symbol}: $${tickerData.price} (Source: ${activeProvider}${
        isFailover ? ' [FAILOVER]' : ''
      })`
    );
    ...
    console.warn(
      `⚠️ [PriceFeedService ALERT] Failed fetching ${stock.symbol}. Error: ${error.message}. Falling back to cache.`
    );
...
export async function getLivePriceWithRetry(
  ...
  try {
    ...
    if (retries > 0) {
      console.warn(
        `[PriceFeedService] Retrying live fetch for ${stock.symbol} in ${delayMs}ms. Retries remaining: ${retries}`
      );
...
async function syncUserPrices(userId: string) {
  ...
  try {
    ...
    const hasNseStocks = stocks.some((s) => isNseSymbol(s.symbol));
    if (hasNseStocks) {
      try {
        await fetchAllNseStocks(false);
      } catch (nseErr: any) {
