...
const up = async ({ context: queryInterface }) => {
    ...
    if (isSqlite) {
        ...
        const [stocksMaster] = (await sequelize.query("SELECT sql FROM sqlite_master WHERE type='table' AND name='Stocks';"));
    ...
    `);
        ...
        const [dailyPricesMaster] = (await sequelize.query("SELECT sql FROM sqlite_master WHERE type='table' AND name='DailyPrices';"));
        ...
        `);
                ...
                const [cols] = (await sequelize.query('PRAGMA table_info(`DailyPrices`);'));
